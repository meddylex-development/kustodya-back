...
namespace Kustodya.Infrastructure.Services
{
    public class PbiEmbedService
    {
        ...
        public static object GetEmbedParam(string accessToken, IOptions<ConfiguracionPowerBiModel> appSettings, Guid reporteId)
        {
            try
            {
                JObject apiResponse = GetEmbedUrl(appSettings.Value.WorkspaceId, reporteId.ToString(), accessToken);
                string embedUrl = apiResponse.GetValue("embedUrl").ToString();
                string datasetId = apiResponse.GetValue("datasetId").ToString();

                apiResponse = GetEmbedToken(datasetId, reporteId.ToString(), accessToken);
