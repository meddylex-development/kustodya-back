...
namespace Kustodya.Infrastructure.Services
{
    ...
    public class ValidadorDeConfiguracionService
    {
        private readonly IOptions<ConfiguracionPowerBiModel> appSettings;

        public ValidadorDeConfiguracionService(IOptions<ConfiguracionPowerBiModel> appSettings)
        {
        ...
        /// <returns></returns>
        public static string ValidateConfig(IOptions<ConfiguracionPowerBiModel> appSettings, Guid reporteId)
        {
            string message = null;
            ...
            else if (string.IsNullOrWhiteSpace(reporteId.ToString()))
            {
                message = "Report Id is not set in appsettings.json file";
            }
            else if (!IsValidGuid(reporteId.ToString()))
            {
