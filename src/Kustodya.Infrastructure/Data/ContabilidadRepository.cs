...
{
    public class ContabilidadRepository<T> : IAsyncContabilidadRepository<T> where T : class
    {
        private readonly ContabilidadContext _dbContext;

        public ContabilidadRepository(ContabilidadContext dbContext)
        {
        ...
        public async Task ReplaceAsync(IReadOnlyList<T> entities, ISqlSpecification<T> specification)
        {
             var entidadeseliminables  = ApplySpecification(specification).ToList();
            _dbContext.Set<T>().RemoveRange(entidadeseliminables);
            await AddRangeAsync(entities.ToList());
        }
    }
}